...
function findLongestWord(sentence) {
  ...
  const sortArray = array.sort((arg1, arg2) => arg2.length - arg1.length);
  return sortArray[0];
}
...
function reverseWords(str) {
  ...
  const reverseArray = array.map((x) => x.split('').reverse().join(''));
...
function invertCase(str) {
  let invert = '';
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === str[i].toLowerCase()) invert += str[i].toUpperCase();
    else invert += str[i].toLowerCase();
  }
  return invert;
}
